...
KEEP_PREV_CNT = 7
FEATURE_CNT = KEEP_PREV_CNT * 2 + 3 + 1  # 7
...
class Board(object):
    ...
    def feature(self):
        ...
        feature_[:, KEEP_PREV_CNT + 1] = (self.color == opp)
        for i in range(KEEP_PREV_CNT):
            feature_[:, i + 1] = (self.prev_color[i] == my)
            feature_[:, KEEP_PREV_CNT + 1 + i + 1] = (self.prev_color[i] == opp)
        if my == 1:
            is_black_turn = 1.0
            is_white_turn = 0.0
        else:
            is_black_turn = 0.0
            is_white_turn = 1.0
        feature_[:, FEATURE_CNT - 2] = is_black_turn
        feature_[:, FEATURE_CNT - 1] = is_white_turn
